...
//! * **Seeding** ([`seed`]) — load an app's `seed/` directory, the rows it
//!   starts life with.
//!
...
pub use seed::{seed, Report as SeedReport};
...
pub enum Filter {
    ...
    AnyContains { columns: Vec<String>, value: String },
}

/// One `ORDER BY` key: a column and a direction.
///
/// The column is a validated field name — the caller (the CRUD layer) decides
/// what a client may sort by; this type only renders it.
...
impl Filter {
    ...
    /// Search `value` against every one of `columns`, matching if any does.
    pub fn any_contains(columns: Vec<String>, value: impl Into<String>) -> Self {
        Filter::AnyContains {
            columns,
            value: value.into(),
        }
    ...
    /// The single column this filter constrains, if it constrains just one.
    fn column(&self) -> Option<&str> {
        match self {
            Filter::Eq { column, .. }
            | Filter::In { column, .. }
            | Filter::Contains { column, .. } => Some(column),
            Filter::AnyContains { .. } => None,
        }
    }
}
...
impl Db {
    ...
    /// `GET /<resource>` — a JSON array of rows, newest first unless `sort`
    /// says otherwise.
    pub async fn list(
        &self,
        r: &Resource,
        ...
        let order = if sort.is_empty() {
            ...
            for key in sort {
                // NULLS LAST in both directions: an empty cell is the least
                // informative row on the page, wherever the arrow points.
                keys.push(format!(
                    "{} {} NULLS LAST",
    ...
    fn render_filter(
        ...
        Ok(match f {
            ...
            Filter::AnyContains { columns, value } => {
                // A search with nothing to search is not "match everything":
                // the caller asked for rows containing a term, and none do.
                if columns.is_empty() {
                    return Ok("false".to_string());
                }
                let escaped = value
                    ...
                    .replace('%', "\\%")
                    .replace('_', "\\_");
                // One bound term shared by every column, so a search over five
                // fields still costs one parameter.
