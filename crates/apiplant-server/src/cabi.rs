...
/// Unlike every other callback this one has nothing to allocate, so it reports
/// through its return code: non-zero delivered, zero nobody listening. A panic
/// is the latter — the chunk did not arrive.
...
///
/// The shape is shared with every other manifest that arrives as JSON — see
/// [`apiplant_abi::manifest_from_json`], which is where the field-by-field
/// rules and their defaults live.
