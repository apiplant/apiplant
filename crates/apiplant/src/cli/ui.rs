...
use super::state::{
    related_label, relation_keys, sign_in_options, AgentChat, Cli, Focus, Form, Main, NavKind,
    SignIn, Team, PAGE,
};
...
fn draw_main(frame: &mut Frame, cli: &Cli, area: Rect) {
    ...
    match &cli.main {
        ...
        Main::Agent(agent) => draw_agent(frame, cli, agent, area, focused),
        Main::Output {
            title,
            body,
            scroll,
...
fn draw_agent(frame: &mut Frame, cli: &Cli, agent: &AgentChat, area: Rect, focused: bool) {
    let Some(meta) = cli.agent(agent.agent) else {
        return;
    };
    let rows = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(if meta.storage { 9 } else { 5 }),
            Constraint::Min(8),
            Constraint::Length(6),
        ])
        .split(area);

    let conversations = if meta.storage {
        format!("{} conversations", meta.label)
    } else {
        format!("{} chat", meta.label)
    };
    let thread_items: Vec<ListItem> = if agent.threads.is_empty() {
        vec![ListItem::new(Line::from(Span::styled(
            agent.browse_note.clone().unwrap_or_else(|| {
                if meta.storage {
                    "No stored conversations yet.".into()
                } else {
                    "This agent keeps only the current chat.".into()
                }
            }),
            Style::default().fg(FAINT),
        )))]
        ...
        agent
            ...
            .iter()
            .map(|thread| {
                let when = thread.updated_at.clone().unwrap_or_default();
                ListItem::new(Text::from(vec![
                    Line::from(thread.title.clone()),
                    Line::from(Span::styled(when, Style::default().fg(FAINT))),
                ]))
            })
            .collect()
    };
    let mut thread_state = ListState::default().with_selected(Some(agent.thread_index));
    frame.render_stateful_widget(
        List::new(thread_items)
            .block(pane(&conversations, focused && !agent.editing))
            .highlight_style(if focused && !agent.editing {
                Style::default().fg(Color::Black).bg(ACCENT)
            } else {
                Style::default().add_modifier(Modifier::REVERSED)
            }),
    ...
    if agent.messages.is_empty() {
        lines.push(Line::from(Span::styled(
            "Start a conversation with `i`, type a message, then press Enter.",
            Style::default().fg(FAINT),
        )));
    } else {
        for message in &agent.messages {
            let who = match message.role.as_str() {
                "assistant" => meta.label.as_str(),
                "system" => "System",
                "tool_call" => "Tool call",
    ...
    frame.render_widget(
        Paragraph::new(lines)
            .block(pane("Messages", focused))
            .wrap(Wrap { trim: false }),
        rows[1],
    );

    let compose_title = match agent.editing {
        true => "Message — Enter send, Esc stop editing",
        false => "Message — i compose, n new chat, Enter open thread",
    };
    let compose = if agent.editing {
        vec![
            Line::from(agent.draft.clone()),
            Line::from(Span::styled("▏", Style::default().fg(ACCENT))),
        ]
    } else if agent.draft.is_empty() {
        vec![Line::from(Span::styled(
            "Press `i` to write to this agent.",
            Style::default().fg(FAINT),
        ))]
    } else {
        vec![Line::from(agent.draft.clone())]
    };
    frame.render_widget(
        Paragraph::new(compose)
            .block(pane(compose_title, focused && agent.editing))
            .wrap(Wrap { trim: false }),
        rows[2],
    );
}
...
fn draw_session(frame: &mut Frame, cli: &Cli, area: Rect, focused: bool) {
    ...
    let mut lines = vec![
        ...
        row("Connected to", &cli.target_label()),
    ...
    // and the line an operator wants when the price list looks right and
    // nothing is being recorded.
    if let Some(billing) = &cli.manifest.billing {
        lines.push(row(
            "Payments",
            &format!(
                "{} · {} · tax {}",
                billing.provider,
                billing.currency.to_uppercase(),
                match billing.automatic_tax {
                    true => "added at checkout",
                    false => "included in the price",
                }
            ),
        ));
        if !billing.webhooks_configured {
            lines.push(row(
                "Webhooks",
                "not configured — checkouts will complete and nothing will be recorded",
            ));
        }
    }
