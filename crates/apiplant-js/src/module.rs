...
/// The module's source. This crate is where it lives; the `typescript/` npm
/// package copies it out of here at pack time, so the file the npm package ships
/// and the one `apiplant.d.ts` beside it describes are the same artifact as the
/// code that runs, rather than two that agree by hand.
pub(crate) const SOURCE: &str = include_str!("../assets/apiplant.js");
