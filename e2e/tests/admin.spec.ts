...
const ORGANIZATION = "Acme Logistics";
...
test("registration creates an account and signs it in", async () => {
  ...
  // With no organisation yet, the dashboard stands the onboarding screen in
  // front of everything rather than showing empty tables.
...
test("creating an organization makes the operator its admin", async () => {
  await page.getByLabel("Name", { exact: false }).first().fill(ORGANIZATION);
  await page.getByRole("button", { name: "Create organization" }).click();

  // Onboarding gives way to the dashboard shell.
  await expect(page.getByRole("button", { name: "Home" })).toBeVisible();
  await settle(page);

  const session = await storedSession(page);
  expect(session.organizationId, "the new organization became the active one").toBeTruthy();
  ...
  expect(rows.map((entry: any) => entry.name)).toContain(ORGANIZATION);

  // The creator is its admin — the membership the server stamped says so.
