 ...
 * Running a function.
 *
 * The point of this screen is that it is *not* a JSON textarea. A function's
 * `input_schema` — which the `function!` macro derives from the handler's own
 * input type — is turned into ordinary labelled inputs, so triggering an action
 * is filling in a short form. A function without a usable schema falls back to
 * raw JSON, because guessing would be worse.
 */

import { For, Show, createEffect, createMemo, createSignal, untrack } from "solid-js";
import { createMutable } from "solid-js/store";
import { Badge, Button, Card, CardHeader, ConfirmDialog, EmptyState, Field, PageTitle, Toggle } from "../ui";
import { api, notify, reportError, session } from "../store";
 ...
 * deep: anything nested is offered as JSON rather than pretended into a form
 * it does not fit.
 */
...
export function ActionPage(props: { fn: FunctionManifest }) {
  ...
  createEffect(() => {
    void props.fn.name;
    const schema = fields();
    // Untracked: reading the draft's keys to clear them would make this effect
    // depend on its own writes.
    untrack(() => {
  ...
  const run = async () => {
    ...
    try {
      ...
      const output = await api(`/functions/${encodeURIComponent(props.fn.name)}`, {
  ...
  const start = () => {
    if (props.fn.confirm) {
      // Validate before asking: being told the form is wrong *after*
      // confirming is a small betrayal.
      try {
        buildBody();
      } catch (error) {
        reportError(error);
...
/**
 * Show a result the way it reads best: a flat object becomes a small
 * definition list, anything else stays as JSON. Most functions return a handful
 * of scalars, and a wall of braces makes that harder to read, not easier.
 */
