 ...
 * sends the operator here. This screen mints an API key for whoever is signed
 * in and posts it straight back, so nobody has to copy a secret between two
 * windows.
 *
 * Two rules make that safe, and neither is negotiable:
 *
 * * **The callback must be loopback.** The address arrives in a link, and a
 *   link is something anyone can send. Refusing anything but `127.0.0.1` /
 *   `[::1]` / `localhost` on plain HTTP means a crafted link cannot turn this
 *   screen into a way to mail someone's key to a stranger.
 * * **A person has to press the button.** Minting on arrival would make merely
 *   *following* a link enough to issue a credential.
 */
 ...
 *
 * Only plain HTTP on a loopback host: the console's listener is not going to
 * have a certificate, and every other origin is somewhere a key must never be
 * sent.
 */
...
export function CliPage() {
  ...
  const connect = async () => {
    ...
    try {
      ...
      // `no-cors` with a plain-text body: the console's listener answers with
      // permissive headers, but this way the handoff does not depend on a
      // preflight succeeding, and we never need to read the response.
      ...
      // The key exists either way — showing it is the difference between a
      // recoverable hiccup and a wasted credential.
      reportError(error);
    } finally {
      setBusy(false);
    }
              ...
              <Show when={issued() && !delivered()}>
                <div class="space-y-2">
                  <p class="text-xs text-faint">
                    ...
                    waiting. Paste this into it instead:
                  </p>
