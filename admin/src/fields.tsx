 ...
 *
 * Two jobs live here. `formatValue` renders a stored value as text: a date as a
 * date, and a foreign key as the name of the record it points at. `FieldEditor`
 ...
 *
 * A reference resolves through the expanded relation the list request
 * requested. When the API did not expand it, or the caller cannot read the
 * target, the raw id is shown rather than a blank.
 */
...
/**
 * Order fields for readability rather than in schema order.
 *
 * Resource fields arrive alphabetically, since they come from an ordered map,
 * which places an `attributes` JSON blob above `name`. The order used here is:
 * the field that names the record, then the columns configured for the list,
 * which already express relative importance, then everything else, with large
 * free-text and JSON inputs last so they do not separate the shorter fields.
 */
 ...
 * stopping at the first, so all errors can be corrected in one pass.
 */
 ...
 *
 * Searches the target resource's display field as you type and shows the
 * selection, so a foreign key never appears as a UUID. When the current value
 * cannot be resolved, because the row was deleted or the resource is not
 * readable, the raw id is shown rather than an empty field.
 */
  ...
  const search = (term: string) => {
    ...
    searchTimer = window.setTimeout(async () => {
      ...
      try {
        ...
        // `field~` is the substring match, so a partial name matches.
