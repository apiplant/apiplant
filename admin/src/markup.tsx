...
function inlineMarkdown(text: string): string {
  const code: string[] = [];
  // Code spans take precedence over every other marker, so extract them first.
 ...
 * rules and paragraphs. Raw HTML in the source remains escaped, so Markdown
 * input cannot become a second way to inject markup.
 */
 ...
 *
 * The preview renders operator-supplied text, which is the classic vector for
 * stored XSS; a permissive preview would execute it inside a session that can
 * edit every record.
 */
...
function highlightHtmlSource(source: string): string {
  ...
  text = text.replace(/&lt;\/?[a-zA-Z][^]*?&gt;/g, (tag) => {
    // A single pass over the tag, so a class name added for the tag itself is
...
/**
 * A textarea with the markup highlighted behind it and the rendered result
 * beside it: one column each on a wide screen, two tabs when space is limited.
 *
 * The highlighting is a `<pre>` beneath a transparent-text textarea, which is
 * the only way to retain native editing (undo, spellcheck, the mobile keyboard)
 * while showing syntax. The two must share every metric affecting layout; see
 * `.markup-input` and `.markup-layer`.
 */
