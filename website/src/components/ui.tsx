...
export function Badge(props: ParentProps<{ tone?: "neutral" | "accent"; class?: string }>) {
  const tones = {
...
/** The apiplant mark: a masked head in dark, a rendered PNG in light. */
export function HeadMark(props: { class?: string }) {
  return <span aria-hidden="true" class={`logo-head ${props.class ?? "h-6"}`} />;
}

/** `apiplant framework`, two-tone the way the studio titles itself. */
export function Wordmark(props: { class?: string }) {
  return (
    <span class={`inline-flex min-w-0 items-center gap-2 ${props.class ?? ""}`}>
      <HeadMark class="h-6 shrink-0" />
