    ...
    <div class="animate-rise mx-auto w-full max-w-5xl px-6 py-6">
      <header class="mb-5">
        ...
        <p class="mt-2 max-w-3xl text-xs leading-relaxed text-muted">
          Everything below is read from this folder. Run it with <Mono>apiplant {props.project.name}</Mono> and
          the API answers at <Mono>{url()}</Mono>
          <Show when={docsUrl()}>
            {" "}
      ...
      <div class="grid gap-3 lg:grid-cols-3">
        <Card class="lg:col-span-2">
          <div class="grid grid-cols-2 divide-x divide-line sm:grid-cols-4">
            <Stat
              label="Resources"
              value={custom().length}
