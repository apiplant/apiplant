  ...
  return (
    <nav class="flex h-full w-64 max-w-[85vw] shrink-0 flex-col border-r border-line bg-surface shadow-xl shadow-black/20 lg:max-w-none lg:bg-surface/40 lg:shadow-none">
      ...
      <div class="mt-1 flex-1 overflow-y-auto px-3 pb-6">
        ...
        <SectionHeader label="Agents" count={agents().length} onAdd={props.onNewAgent} addLabel="New agent" />
        <Show
          when={agents().length}
          fallback={
            <p class="px-2.5 py-1 text-xs leading-relaxed text-faint">
              No agents yet. Add one to expose a named assistant with its own prompt and chat policy.
            </p>
          }
        >
          <For each={agents()}>
            {(entry) => (
              <NavItem
                target={{ kind: "agent", name: entry.name }}
                label={entry.name}
                mono
                dirty={agentDirty(entry)}
                onSelect={props.onNavigate}
                trailing={() => (
