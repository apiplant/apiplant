...
| `thinking` | *(unset)* | Whether to ask the model to think, using each provider's own switch (Anthropic's `thinking`, `chat_template_kwargs.enable_thinking` for local servers). Unset sends nothing and leaves the template's own default alone. Thinking that comes back is always surfaced: `reasoning` stream events, retained on stored messages, and a **Show reasoning** toggle in admin. It is never part of the answer. Individual agents override this with their own `[ai] thinking`. |
