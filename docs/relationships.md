...
field**. Declaring one gives you a real Postgres foreign key, a navigable
`has_many` endpoint on the other side, on-demand inlining with `?expand=`, and
filtering by the foreign key — no extra configuration.
...
`limit`/`offset` paging as a normal list. It's derived purely from the reference
graph — you don't declare `has_many` anywhere.

### Disambiguating multiple references

If a child references the same parent through more than one field (e.g. a
...
query keys are ignored. Multiple filters combine with `AND`. Owner scoping (from
an `owner` permission) is always applied on top.

> Tip: `GET /api/post/{id}/comment` is equivalent to
> `GET /api/comment?post_id={id}`, but also enforces that the parent path is a
> real resource and reads more naturally.
...
All backed by enforced foreign keys with the `on_delete` behaviour you chose.
